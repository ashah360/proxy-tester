...
class App extends Component {
	...
	componentDidMount() {
		...
		if (proxySettings.connected) {
			this.connectToProxyBtn.current.innerHTML = this.state.disconnectText[this.state.region];
	...
	handleConnectBtnClick(e) {
		...
		if (proxySettings.connected) {
			this.connectToProxyBtn.current.innerHTML = this.state.disconnectingText[this.state.region];
			...
			this.connectToProxyBtn.current.innerHTML = this.state.connectText[this.state.region];

			proxySettings.connected = false;
			localStorage.setItem('proxySettings', JSON.stringify(proxySettings));
		}
		else {
			this.connectToProxyBtn.current.innerHTML = this.state.connectingText[this.state.region];
	...
	handlePingTest() {
		this.testPingBtn.current.innerHTML = this.state.pingTestingText[this.state.region];
		...
		const testDomain = this.inputDomain.current.value.trim().replace(/http(s|)(:\/\/|)(www\.|)/, '');
		...
		const proxyURI = 'http://' + ((useAuthCredentials) ? `${proxy.username}:${proxy.password}@` : '') +`${proxy.host}:${proxy.port}`;
		...
		const nextFn = function (text) {
			...
			this.testPingBtn.current.innerHTML = this.state.pingTestText[this.state.region];
			...
			<div className={this.css['container']}>
				<header>
					...
					<div id={this.css['icons']}>
						<img id={this.css['us-flag']} onClick={this.handleRegionChange} src={this.state.flags.US} alt="" />
						<img id={this.css['jp-flag']} onClick={this.handleRegionChange} src={this.state.flags.JP} alt="" />

						<img id={this.css['divider']} src={Divider} alt="" />

						<img id={this.css['theme-switcher']} onClick={this.changeTheme} src={(this.state.theme === 'light') ? Moon : Sun} alt="" />
					</div>
				</header>

				<div className={this.css['form-control-group']}>
					<div style={{ display: 'inline-block', float: 'left', width: '74%' }}>
						<label className={this.css['form-label']} htmlFor="inputProxy">{this.state.proxyText[this.state.region]}</label>
						<input id="inputProxy" className={this.css['input']} onChange={(e) => this.handleProxySettingChange('host', e)} ref={this.inputProxy} type="text"></input>
					</div>

					<div style={{ display: 'inline-block', float: 'right', width: '24%' }}>
						<label className={this.css['form-label']} htmlFor="inputPort">{this.state.portText[this.state.region]}</label>
						<input id="inputPort" className={this.css['input']} onChange={(e) => this.handleProxySettingChange('port', e)} ref={this.inputPort} type="text"></input>
					</div>

					<div style={{ display: 'inline-block', marginTop: '40px', float: 'left', width: '49%' }}>
						<label className={this.css['form-label']} htmlFor="inputUsername">{this.state.usernameText[this.state.region]}</label>
						<input id="inputUsername" className={this.css['input']} onChange={(e) => this.handleProxySettingChange('username', e)} ref={this.inputUsername} type="text"></input>
					</div>

					<div style={{ display: 'inline-block', marginTop: '40px', float: 'right', width: '49%' }}>
						<label className={this.css['form-label']} htmlFor="inputPassword">{this.state.passwordText[this.state.region]}</label>
						<input id="inputPassword" className={this.css['input']} onChange={(e) => this.handleProxySettingChange('password', e)} ref={this.inputPassword} type="password"></input>
					</div>
				</div>

				<span id={this.css['cb-paste']}>{this.state.pasteText[this.state.region]}</span>

				<div className={this.css['form-control-group']}>
					<div style={{ display: 'inline-block', float: 'left', width: '74%' }}>
						<label className={this.css['form-label']} htmlFor="inputDomain">{this.state.domainText[this.state.region]}</label>
						<input id="inputDomain" className={this.css['input']} onChange={(e) => this.handleProxySettingChange('domain', e)} type="text" ref={this.inputDomain} placeholder="kith.com"></input>
					</div>

					<div style={{ display: 'inline-block', float: 'right', width: '24%' }}>
						<label className={this.css['form-label']} htmlFor="inputPing">{this.state.pingText[this.state.region]}</label>
						<input id={this.css['ping']} className={this.css['input']} ref={this.ping} type="text" value="0 ms" disabled></input>
					</div>

					<button style={{ marginTop: '15px' }} className={[this.css['form-btn'], this.css['blue-btn']].join(' ')} onClick={this.handlePingTest} ref={this.testPingBtn}>{this.state.pingTestText[this.state.region]}</button>
					<button style={{ marginTop: '16px', marginBottom: '25px' }} className={[this.css['form-btn'], this.css['grey-btn']].join(' ')} onClick={this.handleConnectBtnClick} ref={this.connectToProxyBtn}>{this.state.connectText[this.state.region]}</button>
				</div>
			</div>
		);
	}
